...
def test_attentative_pooling_msk():
    with torch.no_grad():
        ...
        for _b in range(BATCH_SZ):
            ...
            assert torch.sum( (mask[_b,0, modB_seq_lens[_b]:] == float("-inf")) ) == max(0,modB_seqLen-modB_seq_lens[_b])
            assert torch.sum( (mask[_b, modA_seq_lens[_b]:, 0] == float("-inf")) ) == max(0,modA_seqLen-modA_seq_lens[_b])
...
def test_attentative_pooling():
    with torch.no_grad():
        ...
        tensor_A = torch.rand( (BATCH_SZ,modA_dim,modA_seqLen) , dtype=torch.float, device=device)
        tensor_B = torch.rand( (BATCH_SZ,modB_dim,modB_seqLen) , dtype=torch.float, device=device)
