...
from avssl.model import CascadedSpeechClip

from .base_task import BaseTask


class TrainCascadedSpeechClip(BaseTask):
    def __init__(self):
        super().__init__()
    ...
    def run(self):
        ...
        if self.args.ckpt != "":
            model = CascadedSpeechClip.load_from_checkpoint(self.args.ckpt).to(
                self.args.device
            )
            config = model.config
        else:
            ...
            model = CascadedSpeechClip(config).to(config.device)
        self.config = config
        ...
        # logger = CSVLogger(save_dir=config.trainer.default_root_dir, name="my_exp_name")
