...
class Wav2Vec2Model_cls(BaseFairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--quantize_targets",
            action="store_true",
            help="use the original convnet feature as input to the transformer, quantized convnet features that are reconstruction targets when calculating the loss (w2v2 paper)",
            default=True,
        )
        ...
        parser.add_argument(
            "--quantize_input",
            action="store_true",
            help="quantized all outputs of convnet feature extractor and use them as the input to the transformer",
            default=False,
        )
        ...
        parser.add_argument(
            "--same_quantizer",
            action="store_true",
            help="use the same quantizer to quantize both transformer input and output",
            default=False,
        )
        parser.add_argument(
            "--feature_grad_mult",
        ...
        parser.add_argument(
            "--mask_prob",
            type=float,
            help="probability of replacing a token with mask, default is 0.65, since mask_length=10, this is equivalent to all the tokens in a sequence has 0.065 probability to be starting tokens of a masked span",
            default=0.65,
        )
        ...
        parser.add_argument(
            "--layer_use",
            type=int,
            help="which layer feat to use to input to second tranformer, range from 0 to encoder_layer - 1",
            default=4,
        )
        ...
        parser.add_argument(
            "--diversity_weight",
            type=float,
            help="weight on the diversity loss",
            default=0.1,
        )
    ...
    def __init__(self, args):
        ...
        self.cls_token = torch.nn.Parameter(
            torch.randn((1, 1, args.encoder_embed_dim)), requires_grad=True
        )
    ...
    def compute_preds(self, x, y, negatives):
        ...
        logits = torch.cosine_similarity(x.float(), targets.float(), dim=-1).type_as(
            x
        )  # this gives [N+1, B, T]
    ...
    def forward(
        ...
        if mask:
            ...
            )  # x will be masked, note that mask is some learned embedding (768 dim), not zero!
        ...
        if padding_mask is not None:
            cls_token_padding_mask = torch.zeros((padding_mask.shape[0], 1)).to(
                padding_mask
            )
        ...
        if superb:
            ...
            return {
                "last_hidden_state": all_feats_wo_cls[-1],
                "hidden_states": all_feats_wo_cls,
            }

        features, layer_feats = self.encoder(
...
class TransformerEncoder(nn.Module):
    def __init__(self, args):
        ...
        assert (
            args.layer_use < args.encoder_layers
        ), f"w2v2 only has {args.encoder_layers} layers, but you want layer feat from layer {args.layer_use+1}"
    ...
    def forward(
        ...
        if self.layer_norm_first:
            ...
            if len(temp) == 3:
                ...
                print(
                    f"length of self_attn should be either 2 or 3, but it's {len(temp)}"
                )
            x = self.dropout1(x)
            x = residual + x
