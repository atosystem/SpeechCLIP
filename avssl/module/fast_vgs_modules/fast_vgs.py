...
class ResDavenet(nn.Module):
    def __init__(self):
        ...
        self.layer1 = self._make_layer(
            block, layer_widths[1], layers[0], width=convsize, stride=2
        )
        self.layer2 = self._make_layer(
            block, layer_widths[2], layers[1], width=convsize, stride=2
        )
        self.layer3 = self._make_layer(
            block, layer_widths[3], layers[2], width=convsize, stride=2
        )
        self.layer4 = self._make_layer(
            block, layer_widths[4], layers[3], width=convsize, stride=2
        )
    ...
    def _make_layer(self, block, planes, blocks, width=9, stride=1):
        ...
        layers.append(
            block(
                self.inplanes, planes, width=width, stride=stride, downsample=downsample
            )
        )
...
class DualEncoder(nn.Module):
    ...
    def add_args(parser):
        parser.add_argument(
            "--num_attention_heads",
            type=int,
            help="number of attention heads for visn transformer and cross-modal transformer",
            default=12,
        )
        parser.add_argument(
            "--intermediate_size",
            type=int,
            help="size of feed forward net dimension in visn transformer and cross-modal transformer",
            default=3072,
        )
        parser.add_argument(
            "--hidden_size",
            type=int,
            help="dimension of transformer feature in visn transformer and cross-modal transformer",
            default=768,
        )
        parser.add_argument(
            "--hidden_act",
            type=str,
            help="activation function of visn transformer and cross-modal transformer",
            default="gelu",
        )
        parser.add_argument(
            "--hidden_dropout_prob",
            type=float,
            help="dropout prob for visn transformer and cross-modal transformer",
            default=0.1,
        )
        parser.add_argument(
            "--attention_probs_dropout_prob",
            type=float,
            help="attention dropout prob for visn transformer and cross-modal transformer",
            default=0.1,
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--initializer_range",
            type=float,
            help="range of linear layers (QKV layers) of visn transformer and cross-modal transformer",
            default=0.02,
        )
        parser.add_argument("--layer_norm_eps", type=float, default=1e-12)
        parser.add_argument(
            "--xtrm_layers", type=int, help="number of cross-modal layer", default=5
        )
        parser.add_argument(
            "--trm_layers",
            type=int,
            help="number of visn layer (relational transformer)",
            default=5,
        )
        parser.add_argument(
            "--visual_feat_dim",
            type=int,
            help="input visual feature dim (from faster rcnn)",
            default=2048,
        )
        parser.add_argument(
            "--visual_pos_dim",
            type=int,
            help="input visual positional embedding dim (originally the bounding boxes from faster rcnn)",
            default=4,
        )
        parser.add_argument(
            ...
            action="store_true",
            default=False,
            help="return the attention weight of the first layer of the first x_layer, i.e. audio attends to image feats [b,heads,T_audio,T_image]",
        )
        ...
        parser.add_argument(
            ...
            type=float,
            default=None,
            help="the weight on w2v2 loss on audios of spokencoco/places/flickr8k",
        )
        parser.add_argument("--libri_w2v2_weight", type=float, default=None)
        parser.add_argument(
            "--coarse_to_fine_retrieve", action="store_true", default=False
        )
        parser.add_argument(
            "--fb_w2v2_weights_fn",
            type=str,
            help="the path of w2v2 small model trained by FAIR",
            default=None,
        )
    ...
    def __init__(self, args):
        ...
        if (
            self.args.caption_w2v2_weight == None
            and self.args.libri_w2v2_weight == None
        ):
        ...
        self.visual_cls_token = torch.nn.Parameter(
            torch.randn((1, 1, args.hidden_size)), requires_grad=True
        )
    ...
    def forward_image(self, visual_feats, visual_pos, visual_attention_mask):
        ...
        visual_feats = torch.cat(
            [self.visual_cls_token.repeat(visual_feats.shape[0], 1, 1), visual_feats],
            dim=1,
        )
    ...
    def forward_audio(
        self, audio_feats, audio_attention_mask, test=False, target_list=None
    ):
        if test:
            self.conv1_trm1_trm3.eval()
            trm13_out = self.conv1_trm1_trm3(
                audio_feats,
                padding_mask=audio_attention_mask,
                mask=False,
                features_only=True,
                tgt_layer=self.args.layer_use,
            )
            losses = {}
        else:
            ...
            if self.no_caption_audio_loss:
                ...
                trm13_out = self.conv1_trm1_trm3(
                    audio_feats, padding_mask=audio_attention_mask, mask=True
                )
                losses = w2v2_loss(
                    self.conv1_trm1_trm3, trm13_out, self.args, suffix="caption"
                )
        ...
        attention_mask = torch.arange(len(audio_feats[0])).unsqueeze(0).to(
            audio_feats.device
        ) >= nframes.unsqueeze(1)
        ...
        cls_token_padding_mask = torch.zeros((attention_mask.shape[0], 1)).to(
            attention_mask
        )
        ...
        )  # 0.0 is what we want to attend, -10000. is what we don't want to attend
    ...
    def forward(
        ...
        if forward_libri:
            ...
            visual_feats, visual_cls = self.forward_image(
            ...
            (
                ...
                audio_feats, attention_mask, test, target_list=target_list
            )
            return (
                audio_feats,
                audio_cls,
            ...
            (
                ...
                extended_audio_attention_mask,
                losses,
            ) = self.forward_audio(audio_feats, attention_mask, target_list=target_list)
            return (
                audio_feats,
                audio_cls,
                extended_audio_attention_mask,
...
class CrossEncoder(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.args = args
        self.xtrm = nn.ModuleList([LXMERTXLayer(args) for _ in range(args.xtrm_layers)])
    ...
    def forward(
        self,
        audio_feats_square,
        extended_audio_attention_mask_square,
        visual_feats_square,
        extended_visual_attention_mask_square=None,
        ...
        for layer_module in self.xtrm:
            audio_feats_square, visual_feats_square = layer_module(
                ...
                extended_audio_attention_mask_square,
                visual_feats_square,
                extended_visual_attention_mask_square,
            )
        cls_token = torch.cat(
            [audio_feats_square[:, 0], visual_feats_square[:, 0]], dim=-1
        )
