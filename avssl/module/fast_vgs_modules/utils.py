...
def w2v2_loss(model, w2v2_out, args, suffix):
    x = w2v2_out["features"][w2v2_out["mask_indices"]].view(
        w2v2_out["features"].size(0), -1, w2v2_out["features"].size(-1)
    )
    ...
    if args.trim_mask:
        logits = model.compute_preds_trim_mask(
            x, y, negs
        )  # [num_negs+1, B, T_pred], which is consineSim between prediction and target
        ...
        logits = model.compute_preds(
            x, y, negs, w2v2_out["mask_indices"]
        )  # # [n_negatives+1, T_pred1+T_pred2+...+T_predB], which is consineSim between prediction and target
...
def Margin_InfoNCE_loss(S, margin, img_id=None):
    ...
    S = S - deltas
    very_neg = torch.tensor(-10000.0).to(S)
    if img_id is not None:
        img_id_equal_matrix = torch.from_numpy((img_id[:, None] == img_id[None, :])).to(
            S
        )
        diag_mask = (-1.0 * torch.eye(S.size(0)).to(S.device)) + torch.ones_like(S)
        mask = (
            img_id_equal_matrix * diag_mask * very_neg
        )  # diag are all 0; for non-diag entries, -10000 if corresponding img_ids' are equal, otherwise 0
...
class Last_LXMERTXLayer(nn.Module):
    ...
    def self_att(self, visn_input, visn_attention_mask, return_attention_weight=False):
        # Self Attention
        # audio_att_output = self.audio_self_att(audio_input, audio_attention_mask)[0]
        # visn_att_output = self.visn_self_att(visn_input, visn_attention_mask)[0]
        # not [0], maybe this is only when we use transformer's bert modules
        ...
        # return audio_att_output, visn_att_output
        if return_attention_weight:
            visn_att_output, attention_weight = self.visn_self_att(
                visn_input,
                visn_input,
                visn_attention_mask,
                return_attention_weight=True,
    ...
    def forward(
        ...
        if return_attention_weight:
            ...
            visn_att_output = self.self_att(visn_att_output, visn_attention_mask)
            visn_output = self.output_fc(visn_att_output)
            return visn_output
